...
export function normalizeMount(
  ...
  return {
    ...
    icon: asString(r.icon) ?? asString(r.iconName),
    iconName: asString(r.iconName),
    iconFileId: asNumber(r.iconFileId) ?? asNumber(r.iconFileID),
    image: asString(r.image),
    modelId: asNumber(r.modelId),
    itemId: asNumber(r.itemId),
    comment: asString(r.comment),
    requirements: asString(r.requirements),
    internalIds:
      r.internalIds && typeof r.internalIds === 'object'
        ? (r.internalIds as Record<string, number>)
        : undefined,
...
function normalizeFlatSource(r: Record<string, unknown>, faction: Faction): MountSource {
  ...
  return {
    ...
    npc: asString(r.npc),
    zone: asString(r.zone),
    subzone: asString(r.subzone),
    continent: asString(r.continent),
    difficulty: asString(r.difficulty),
    availability: asString(r.availability),
    vendor: asString(r.vendor),
    profession: asString(r.profession),
    event: asString(r.event),
    cost: asString(r.cost),
    requirement: asString(r.requirements) ?? sourceText,
    faction,
    coordinates,
  }
}
